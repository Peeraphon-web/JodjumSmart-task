...
export default function Dashboard({ session }) {
  ...
  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} transition-all duration-500 pb-12 font-sans`}>
      ...
      {/* --- LEVEL UP --- */}
      {showLevelUp && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center pointer-events-none text-white">
          <div className="absolute inset-0 bg-yellow-400/10 backdrop-blur-[2px]"></div>
          <div className="relative animate-in zoom-in-50 duration-500 flex flex-col items-center">
      ...
      {/* --- TOP NAVIGATION --- */}
      <nav className={`sticky top-0 z-40 ${theme.card} border-b border-gray-100 mb-8 px-4 py-3 shadow-sm`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <button onClick={() => setIsSettingsOpen(true)} className="p-2 hover:bg-gray-100 rounded-full border border-gray-50 text-gray-400 hover:text-black transition-all">
            <Settings size={20} />
          </button>
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8 pb-20">
        <div className="lg:col-span-7 space-y-6 order-1">
          <div className={`${theme.card} rounded-[2rem] p-6 shadow-sm border border-gray-100 flex flex-col sm:flex-row items-center gap-6 relative overflow-hidden bg-cover bg-center transition-all duration-700`} style={{ backgroundImage: theme.profileBg }}>
            <div className={`w-20 h-20 shrink-0 ${theme.levelBox} rounded-3xl flex items-center justify-center text-3xl font-black text-white shadow-lg z-10`}>
              {profile?.current_level || 1}
            </div>
            <div className="flex-1 w-full z-10">
              <div className="flex items-center gap-2">
                <h2 className="text-2xl font-black italic tracking-tighter uppercase">Level Explorer</h2>
                {profile?.is_premium && (
                <div className="bg-yellow-100 text-yellow-600 px-3 py-1 rounded-full flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className={`${theme.card} p-5 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 relative overflow-hidden`}>
              <div className={`absolute -right-4 -bottom-4 ${doneToday >= 3 ? "text-yellow-400 opacity-20 scale-125" : "text-orange-500 opacity-10"}`}><Zap size={80} fill="currentColor" /></div>
              ...
              <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{doneToday >= 3 ? "Supercharged" : "Streak"}</p><span className="text-2xl font-black">{profile?.streak_count || 0} Days</span></div>
            </div>
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex gap-1.5 bg-gray-50 p-1 rounded-2xl">
                {["high", "medium", "low"].map((p) => (
                  <button key={p} onClick={() => (!profile?.is_premium && p !== "medium") ? alert("Premium Only!") : setPriority(p)} className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase transition-all ${priority === p ? "bg-black text-white" : "text-gray-400"}`}>{p} {!profile?.is_premium && p !== "medium" && <Lock size={8} />}</button>
                ))}
              </div>
              <button onClick={() => !profile?.is_premium ? alert("Premium Only!") : setIsAddingCat(true)} className="flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-2xl text-[9px] font-black uppercase text-gray-500"><FolderPlus size={14} /> Folder</button>
            </div>
            ...
            <div className="divide-y divide-gray-50">
              {filteredTasks.map((task) => (
                <div key={task.id} className="group flex items-center gap-4 p-5 hover:bg-gray-50/60 transition-all">
                  ...
                  <span className={`flex-1 text-[17px] truncate ${task.status === "done" ? "line-through text-gray-400 italic" : "font-bold"}`}>{task.title}</span>
          ...
          <div className={`${theme.card} rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden`}>
            ...
            {showHistory && (
              <div className="p-4 space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                {historyTasks.map((t) => (
                  <div key={t.id} className="flex items-center gap-3 p-3 bg-gray-50/50 rounded-2xl text-black">
                    <CheckCircle2 size={14} className="text-green-500" />
                    <span className="text-sm flex-1 truncate line-through text-gray-400">{t.title}</span>
                    <button onClick={() => deleteHistoryPermanently(t.id)} className="text-gray-300 hover:text-red-500"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            )}
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in-95 duration-200 text-black">
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-[1.5rem] border border-gray-100 mb-6">
              <div className={`w-14 h-14 ${theme.levelBox} text-white rounded-2xl flex items-center justify-center font-black text-xl shadow-lg`}>{profile?.current_level}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  {isEditingUsername ? (
                    <input autoFocus className="bg-transparent border-b-2 border-black font-black text-sm outline-none w-full" value={tempUsername} onChange={(e) => setTempUsername(e.target.value)} onKeyDown={(e) => e.key === "Enter" && updateUsername()} />
                  ) : (
                    <>
                      <p className="text-sm font-black truncate">{profile?.username || "Explorer"}</p>
                      <button onClick={() => setIsEditingUsername(true)} className="p-1 text-gray-300 hover:text-black"><Edit2 size={12} /></button>
                      {profile?.is_premium && <Crown size={12} className="text-yellow-500" fill="currentColor" />}
                    </>
                  )}
                </div>
            ...
            {/* --- THEME SELECTOR (SHOW ONLY IF PREMIUM) --- */}
            {profile?.is_premium && (
              <div className="mb-6">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Custom Themes</p>
                  ...
                  {Object.keys(themes).map((t) => (
                    <button
                      ...
                      className={`flex-1 py-3 rounded-xl border-2 transition-all flex items-center justify-center ${
                        currentTheme === t ? "border-black scale-105 shadow-md" : "border-transparent bg-gray-50 opacity-60"
                      }`}
                    >
                      <span className="text-xl">{themes[t].icon}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <button onClick={handleForgotPassword} className="w-full py-4 text-xs font-black text-gray-500 bg-gray-50 rounded-2xl mb-4 hover:bg-gray-100 transition-all flex items-center justify-center gap-2 uppercase tracking-widest"><KeyRound size={16} /> Reset Password</button>
            ...
            {!profile?.is_premium ? (
              ...
              <button onClick={handleManageSubscription} disabled={isOpeningPortal} className="w-full bg-black text-white py-4 rounded-2xl font-black italic mb-4 flex items-center justify-center gap-2">
                {isOpeningPortal ? <Loader2 className="animate-spin" /> : <><CreditCard size={18} /> MANAGE SUBSCRIPTION</>}
              </button>
            )}
