...
export default function Dashboard({ session }) {
  ...
  const expPercentage = useMemo(
    () => (profile?.exp_points ? profile.exp_points % 100 : 0),
    [profile]
  );
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center pointer-events-none overflow-hidden text-white">
          ...
          <div className="relative animate-in zoom-in-50 duration-500 ease-out flex flex-col items-center">
            <div className="bg-black text-white px-10 py-6 rounded-[2.5rem] shadow-2xl flex flex-col items-center border-t-2 border-yellow-300/50">
              <Zap
                size={36}
                className="text-yellow-400 fill-yellow-400 animate-bounce mb-2"
              />
      ...
      <main className="max-w-6xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8 pb-20">
        <div className="lg:col-span-7 space-y-6 order-1 text-black">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div
              className={`${theme.card} p-5 rounded-[2rem] border border-gray-100 shadow-sm flex items-center gap-4 relative overflow-hidden transition-all`}
            >
              <div
                className={`absolute -right-4 -bottom-4 transition-all ${
                  doneToday >= 3
                    ? "text-yellow-400 opacity-20 scale-125"
                    : "text-orange-500 opacity-10"
                }`}
              >
              ...
              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                  {doneToday >= 3 ? "Supercharged" : "Current Streak"}
                </p>
            ...
            <div
              className={`${theme.card} p-5 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col justify-center gap-2`}
            >
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                onClick={() => {
                  if (!profile?.is_premium) return alert("Premium Only!");
                  setIsAddingCat(true);
                }}
                className="flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-2xl text-[9px] font-black uppercase text-gray-500"
              >
            ...
            <form onSubmit={addTask} className="relative">
              <input
                ...
                className="w-full pl-6 pr-16 py-4 bg-gray-50 rounded-[1.5rem] outline-none text-lg font-bold"
                value={newTask}
                onChange={(e) => setNewTask(e.target.value)}
              />
              <button className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-black text-white rounded-2xl flex items-center justify-center">
                <Plus size={24} />
              </button>
            </form>
          </div>
          ...
          {/* Task List */}
          <div
            className={`${theme.card} rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden min-h-[400px]`}
          >
            <div className="divide-y divide-gray-50">
              {filteredTasks.map((task) => (
                <div
                  key={task.id}
                  className="group flex items-center gap-4 p-5 hover:bg-gray-50/60 transition-all"
                >
                  <div
                  ...
                  <span
                    className={`flex-1 text-[17px] truncate ${
                      task.status === "done"
        ...
        <div className="lg:col-span-5 space-y-8 relative order-2">
          {!profile?.is_premium && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-20 flex flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-gray-200 p-6 text-center">
              ...
              <h3 className="font-black italic text-xl mb-4 uppercase">
                HISTORY LOCKED
              </h3>
              <button
                onClick={handleUpgrade}
                className="bg-yellow-400 text-white px-8 py-3 rounded-2xl font-black italic shadow-lg"
              >
                UNLOCK NOW
              </button>
            </div>
          ...
          <div
            className={`${theme.card} rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden`}
          >
            <button
              onClick={() => setShowHistory(!showHistory)}
              className="w-full p-6 border-b border-gray-50 flex justify-between items-center text-black font-black text-[10px] uppercase tracking-widest"
            >
              <div className="flex items-center gap-2">
            ...
            {showHistory && (
              <div className="p-4 space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                {historyTasks.map((t) => (
                  <div
                    key={t.id}
                    className="flex items-center gap-3 p-3 bg-gray-50/50 rounded-2xl text-black"
                  >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[2.5rem] shadow-2xl p-8 animate-in zoom-in-95 duration-200">
              ...
              <div className="flex-1 min-w-0">
                {isEditingUsername ? (
                  <div className="flex items-center gap-2">
                    <input
                      autoFocus
                      className="bg-transparent border-b-2 border-black font-black text-sm outline-none w-full"
                      value={tempUsername}
            ...
            <button
              ...
              className="w-full py-4 text-xs font-black text-gray-500 bg-gray-50 rounded-2xl mb-4 hover:bg-gray-100 transition-all flex items-center justify-center gap-2 uppercase tracking-widest"
            >
            ...
            {!profile?.is_premium && (
              <button
                ...
                className="w-full bg-gradient-to-r from-[#FFB800] to-[#FF3D00] text-white py-4 rounded-2xl font-black italic shadow-lg mb-4 flex items-center justify-center gap-2"
              >
                {isUpgrading ? (
                  <Loader2 className="animate-spin" />
                ) : (
                  <>
                    <Crown size={18} fill="white" /> UNLOCK PRO FEATURES
                  </>
                )}
              </button>
            )}
            ...
            <button
              ...
              className="w-full py-4 text-xs font-black text-red-500 bg-red-50 rounded-2xl hover:bg-red-500 hover:text-white transition-all uppercase tracking-widest"
            >
              LOGOUT
            </button>
          </div>
      ...
      {isAddingCat && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm shadow-2xl">
            <h3 className="font-black italic text-lg mb-6 uppercase">
              New Folder
            </h3>
            <input
              autoFocus
              className="w-full p-4 bg-gray-100 rounded-3xl outline-none mb-6 font-bold"
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-4 text-[12px] font-black uppercase opacity-40"
              >
                Cancel
              </button>
              <button
                onClick={addCategory}
                className="flex-1 py-4 bg-black text-white rounded-2xl text-[12px] font-black uppercase shadow-lg"
              >
                Create
              </button>
            </div>
